...
def client_worker(params):
    ...
    for _ in range(epochs):
        for x, y in loader:
            ...
            # 假设模型在前向传播中支持 return_feat 参数，如果不支持则需要修改模型定义
            # 在本框架中，CNN/ResNet 等通常可以通过 extractor 获取特征
...
class Server(BaseServer):
    ...
    def __init__(self, args):
        ...
        self.S_cache = [
            torch.zeros(args.n_class, args.feature_dim) for _ in range(self.num_clients)
        ]
        self.W_cache = [torch.zeros(args.n_class, 1) for _ in range(self.num_clients)]
        self.consensus_P = [
            torch.zeros(args.n_class, args.feature_dim) for _ in range(self.num_clients)
        ]
    ...
    def fit(self):
        ...
        for r in range(self.rounds):
            ...
            for i in selected_clients:
                res = results[i]
                total_loss += res["avg_loss"]
                self.clients_state[i] = res["state"]
                self.S_cache[i] = res["S"]
                self.W_cache[i] = res["W"]
            ...
            W_tensor = W_flat.view(self.num_clients, self.args.n_class, 1)
            S_tensor = S_flat.view(
                self.num_clients, self.args.n_class, self.args.feature_dim
            )
