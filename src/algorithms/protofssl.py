...
def _group_prototypes(model, x, idx, y, num_class, feature_dim, device):
    """
...
def sample_per_class(labeled_idx_by_class, support_size, query_size):
    """
    每类随机采样支持集 S / 查询集 Q（不替换）。
    某类有标签样本不足时取全部可用。
    ...
    for k, idx in labeled_idx_by_class.items():
        ...
        s = min(support_size, n)
        q = min(query_size, n - s)
...
def train(params):
    ...
    model = get_model(
        model_name, dataset_name, num_class, feature_dim
    ).to(device)
    ...
    for _ in range(epochs):
        ...
        sup_idx, sup_y, que_idx, que_y = sample_per_class(
            labeled_idx_by_class, support_size, query_size
        )

        # 本地原型（基于支持集, Eq.2）
        C_local = _group_prototypes(
            model, train_set.x, sup_idx, sup_y, num_class, feature_dim, device
        )

        loss = None

        # 监督项（Eq.8 第一项）：标注查询集 → 本地原型距离概率 → CE(真实标签)
        if len(que_idx) > 0:
            f_q = model.extractor(train_set.x[que_idx].to(device))  # [B_q, D]
            p_ii_x = proto_logits(f_q, C_local, 1.0)  # Eq.(6) j=i
            loss = F.cross_entropy(p_ii_x, que_y.to(device))

        # 无监督项（Eq.8 第二项）：伪标签 → 本地原型距离概率 → CE(伪标签)
        if helper_protos is not None and len(unlabeled_idx) > 0:
            u_idx = unlabeled_idx[
                torch.randperm(len(unlabeled_idx))[:unlabeled_query_size]
            ]
            f_u = model.extractor(train_set.x[u_idx].to(device))  # [B_u, D]
            p_bar_u = pseudolabel(f_u, helper_protos, sharpen_T)  # [B_u, K] soft
            p_ii_u = proto_logits(f_u, C_local, 1.0)  # Eq.(6) j=i
            loss_unsup = F.cross_entropy(p_ii_u, p_bar_u)
            loss = loss_unsup if loss is None else loss + lambda_ * loss_unsup

        if loss is None:
            # 既无标注查询又无辅助原型时跳过本步更新（极端情况）
            continue
...
class Server(BaseServer):
    ...
    def fit(self):
        ...
        for r in range(self.rounds):
            ...
            print(
                f"Global Proto Accuracy: {self.acc_proto[-1]:.2f}%, "
