...
class Params(BaseParams):
    ...
    proto_reliability: torch.Tensor
    lambda_p: float
    lambda_s: float
    lambda_u: float
    tau: float
...
def client_eval_and_diagnose_worker(p: ClientEvalParams):
    """客户端使用当前全局模型与全局原型，并行完成类别可靠性统计、类别半径估计、相对几何统计与无标签诊断。

    对有标签样本仅进行一次前向特征提取：
      1. 用优化后的目标原型 global_protos 计算类别预测精确率 (混淆统计)；
    ...
    # 1. 有标签数据处理 (单次前向同时产出类别可靠性、类别半径、相对几何)
    ...
    return {
        ...
        "labeled_n_pred": labeled_n_pred.cpu().detach().clone(),
        "labeled_n_correct": labeled_n_correct.cpu().detach().clone(),
    }
...
def train(p: Params):
    ...
    for _ in range(p.epochs):
        for labeled_batch, unlabeled_batch in iterate_ssl_batches(loaders):
            ...
            # 原型校准损失：直接向量化计算，有样本时除以有效类别数，无样本时不产生梯度且不除以0
            diff_per_sample = mse_distance(features_l, global_protos).gather(
                1, y_l.unsqueeze(1)
            ).squeeze(1)
            valid_labeled = global_valid[y_l]
            v_count = valid_labeled.sum()
            loss_cal = (
            ...
            if can_pseudo_label:
                with torch.no_grad():
                    ...
                    sample_reliability = (1.0 - proto_q_values).clamp(
                        min=0.0, max=1.0
                    )
                    class_reliability = proto_reliability[pseudo_labels]
                    weights = sample_reliability * class_reliability
                ...
                loss_u = (loss_per_sample * weights.detach()).mean()
            ...
            with torch.no_grad():
                ...
                if can_pseudo_label:
                    is_correct = pseudo_labels.eq(y_u)

                    train_stats[10] += weights.sum().double()
                    train_stats[11] += (weights * is_correct.float()).sum().double()
                    train_stats[12] += is_correct.sum().double()
                    train_stats[13] += (weights * missing_u.float()).sum().double()
                    train_stats[14] += (
                        (weights * is_correct.float() * missing_u.float()).sum().double()
                    )
                    train_stats[15] += (is_correct & missing_u).sum().double()

                    # 按预测类别累加样本级诊断
                    class_diag_stats[:, 0] += torch.bincount(
                        pseudo_labels, minlength=p.num_class
                    ).double()
                    class_diag_stats[:, 1].index_add_(
                        0, pseudo_labels, proto_q_values.double()
                    )
                    class_diag_stats[:, 2].index_add_(
                        0, pseudo_labels, sample_reliability.double()
                    )
                    class_diag_stats[:, 3].index_add_(
                        0, pseudo_labels, weights.double()
                    )
                    class_diag_stats[:, 4].index_add_(
                        0, pseudo_labels, (weights * is_correct.float()).double()
                    )
    ...
    proto_diag = {
        "unlabeled_total": int(stats_cpu[8]),
        "missing_total": int(stats_cpu[9]),
        "unlabeled_weight_sum": float(stats_cpu[10]),
        "unlabeled_weighted_correct": float(stats_cpu[11]),
        "unlabeled_raw_correct": int(stats_cpu[12]),
        "missing_weight_sum": float(stats_cpu[13]),
        "missing_weighted_correct": float(stats_cpu[14]),
        "missing_raw_correct": int(stats_cpu[15]),
        "class_diag_stats": class_diag_stats.cpu().detach().clone(),
    }
...
class Server(BaseServer):
    def __init__(self, args):
        ...
        self.proto_reliability = torch.zeros(self.num_class)
        self.proto_reliability_by_round = []
        ...
        metric_names = (
            ...
            "class_support",
            "proto_reliability_by_round",
            "unlabeled_mean_weight",
            "unlabeled_weight_sum",
            "unlabeled_raw_acc",
            "unlabeled_weighted_acc",
            "missing_mean_weight",
            "missing_weighted_acc",
    ...
    def _compute_proto_reliability(self, eval_results, selected):
        total_pred = torch.zeros(self.num_class, dtype=torch.long)
        total_correct = torch.zeros(self.num_class, dtype=torch.long)
        for cid in selected:
            total_pred += eval_results[cid]["labeled_n_pred"]
            total_correct += eval_results[cid]["labeled_n_correct"]

        reliability = torch.zeros(self.num_class)
        for c in range(self.num_class):
            if bool(self.global_valid[c]) and total_pred[c] > 0:
                reliability[c] = total_correct[c].float() / total_pred[c].float()
            else:
                reliability[c] = 0.0

        self.proto_reliability = reliability
        self.proto_reliability_by_round.append(self.proto_reliability.tolist())
    ...
    def fit(self):
        ...
        for index in range(self.rounds):
            ...
            train_proto_reliability = self.proto_reliability.clone()
            params = []
                ...
                params.append(
                    Params(
                        **asdict(base),
                        ...
                        proto_reliability=self.proto_reliability.clone(),
            ...
            # 5. 服务端汇总计算类别原型可靠性 r_c
            self._compute_proto_reliability(eval_results, selected)
            ...
            # 统计客户端本轮训练中连续可靠性权重的实际执行效果
            unlabeled_total = sum(
                results[cid]["proto_diag"]["unlabeled_total"] for cid in selected
            )
            missing_total = sum(
                results[cid]["proto_diag"]["missing_total"] for cid in selected
            )
            weight_sum = sum(
                results[cid]["proto_diag"]["unlabeled_weight_sum"] for cid in selected
            )
            weighted_correct = sum(
                results[cid]["proto_diag"]["unlabeled_weighted_correct"]
                for cid in selected
            )
            raw_correct = sum(
                results[cid]["proto_diag"]["unlabeled_raw_correct"] for cid in selected
            )
            missing_weight_sum = sum(
                results[cid]["proto_diag"]["missing_weight_sum"] for cid in selected
            )
            missing_weighted_correct = sum(
                results[cid]["proto_diag"]["missing_weighted_correct"]
                for cid in selected
            )

            mean_weight = weight_sum / max(1, unlabeled_total)
            weighted_acc = (
                100.0 * weighted_correct / max(1e-12, weight_sum)
                if weight_sum > 0
                else 0.0
            )
            raw_acc = 100.0 * raw_correct / max(1, unlabeled_total)
            missing_mean_weight = missing_weight_sum / max(1, missing_total)
            missing_weighted_acc = (
                100.0 * missing_weighted_correct / max(1e-12, missing_weight_sum)
                if missing_weight_sum > 0
                else 0.0
            )

            self.unlabeled_mean_weight.append(mean_weight)
            self.unlabeled_weight_sum.append(weight_sum)
            self.unlabeled_raw_acc.append(raw_acc)
            self.unlabeled_weighted_acc.append(weighted_acc)
            self.missing_mean_weight.append(missing_mean_weight)
            self.missing_weighted_acc.append(missing_weighted_acc)

            class_diag_all = sum(
                results[cid]["proto_diag"]["class_diag_stats"] for cid in selected
            )
    ...
    def _record_and_print(
        ...
        print(
            "客户端训练连续可靠性执行统计（本轮训练）：\n"
            f"无标签平均监督权重={self.unlabeled_mean_weight[-1]:.4f}，无标签权重总和={self.unlabeled_weight_sum[-1]:.2f}\n"
            f"伪标签准确率={self.unlabeled_raw_acc[-1]:.2f}%，加权伪标签准确率={self.unlabeled_weighted_acc[-1]:.2f}%\n"
            f"缺失类别：样本数={missing_total}，平均监督权重={self.missing_mean_weight[-1]:.4f}，加权伪标签准确率={self.missing_weighted_acc[-1]:.2f}%"
        )

        # 打印客户端本轮训练实际使用的类别原型精确率与每类无标签详细分解
        class_decomp_lines = []
        for c in range(self.num_class):
            r_c = float(train_proto_reliability[c].item())
            n_c = int(class_diag_all[c, 0].item())
            if n_c > 0:
                mean_q_c = float(class_diag_all[c, 1].item()) / n_c
                mean_s_c = float(class_diag_all[c, 2].item()) / n_c
                mean_w_c = float(class_diag_all[c, 3].item()) / n_c
                w_corr_c = float(class_diag_all[c, 4].item())
                w_acc_c = (
                    w_corr_c / max(1e-12, float(class_diag_all[c, 3].item()))
                ) * 100.0
            else:
                mean_q_c, mean_s_c, mean_w_c, w_acc_c = 0.0, 0.0, 0.0, 0.0
            class_decomp_lines.append(
                f"c{c}: proto_precision={r_c:.4f} | pseudo_count={n_c}, "
                f"mean_q={mean_q_c:.4f}, mean_sample_reliability={mean_s_c:.4f}, "
                f"mean_weight={mean_w_c:.4f}, weighted_acc={w_acc_c:.2f}%"
            )

        print(
            "客户端本轮训练使用的类别原型精确率与无标签分解：\n"
            + "\n".join(class_decomp_lines)
        )

        # 打印本轮刚评估出的最新目标原型可靠性 (供下一轮客户端使用)
        next_r_strs = [
            f"c{c}={self.proto_reliability[c]:.4f}"
            for c in range(self.num_class)
        ]
        print(
            "下一轮目标原型类别可靠性（本轮评估得出，供下一轮客户端训练使用）：\n"
            + " | ".join(next_r_strs)
        )
        ...
        if has_q_data:
            ...
            print(
                "目标原型 q 可靠性诊断（仅用于特征空间质量观察，不参与训练决策）：\n"
    ...
    def _print_candidate_diagnostics(self):
        ...
        print(
            ...
            f"无标签连续加权 CE（按全部无标签样本归一化）={self.loss_u[-1]:.6f}\n"
        ...
        self.deal_save(
            metrics,
            {
                ...
                "aux": {
                    ...
                    "proto_reliability": self.proto_reliability,
                },
            },
        )
