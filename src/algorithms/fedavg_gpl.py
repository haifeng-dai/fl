...
def fixmatch_loss(logits_l, logits_u_s, teacher_logits_u, y_l, confidence, lam):
    """使用冻结全局 teacher 伪标签的 FixMatch 损失。"""
    loss_x = F.cross_entropy(logits_l, y_l)
    with torch.no_grad():
        teacher_probs = torch.softmax(teacher_logits_u, dim=1)
        max_probs, pseudo_targets = teacher_probs.max(dim=1)
...
def train(p: Params):
    ...
    for _ in range(p.epochs):
        for labeled_batch, unlabeled_batch in iterate_ssl_batches(loaders):
            ...
            logits_l = student_logits[:labeled_batch_size]
            logits_u_s = student_logits[labeled_batch_size:]
            with torch.no_grad():
                teacher_logits_u = teacher(x_u_w)
    ...
    state = clone_cpu_state(student.state_dict())
    return {
        "state": state,
...
class Server(BaseServer):
    ...
    def fit(self):
        ...
        for round_id in range(self.rounds):
            ...
            teacher_state = clone_cpu_state(self.model.state_dict())
            parameters = [
                Params(
                    **asdict(base),
            ...
            print(
                f"\n--- FedAvg-GPL Round {round_id + 1}/{self.rounds} ---"
