...
class Server(BaseServer):
    ...
    def fit(self):
        ...
        for r in range(self.rounds):
            ...
            if r == 0:
                ...
                for i in range(self.num_clients):
                    print(f"Client {i} | Local GSD: {all_gsds[i].item():.6f} | [Warmup] Force Triggered")
            elif r == 1:
                # 第二轮全员触发（过渡），同时初始化 local_gsd_ema = GSD_i
                local_trigger.fill_(True)
                for i in range(self.num_clients):
                    self.local_gsd_ema[i] = all_gsds[i]
                    print(f"Client {i} | Local GSD: {all_gsds[i].item():.6f} | [Transition] Force Triggered, EMA initialized to {self.local_gsd_ema[i].item():.6f}")
            else:
                for i in range(self.num_clients):
                    # 更新 local_gsd_ema（追踪 GSD_i 自身历史）
                    self.local_gsd_ema[i] = self.eta * self.local_gsd_ema[i] + (1.0 - self.eta) * all_gsds[i]

                    current_gamma = self.local_gsd_ema[i]

                    print(f"Client {i} | Local GSD: {all_gsds[i].item():.6f} | Own EMA: {self.local_gsd_ema[i].item():.6f} | Gamma: {current_gamma.item():.6f}")
            ...
            print(f"Avg Loss: {self.loss[-1]:.4f}, Acc: {self.acc[-1]:.2f}%, Proto Acc: {self.acc_proto[-1]:.2f}%")
    ...
    def save(self):
        f = {"acc": self.acc, "loss": self.loss, "acc_proto": self.acc_proto, "num_triggered": self.num_triggered_log, "triggered_ids": self.triggered_ids_log, "state_dict": self.clients_state, "consensus_P": self.consensus_P}
        self.deal_save(f)
