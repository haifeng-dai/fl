...
def client_worker(params):
    ...
    valid_mask = consensus_P.abs().sum(dim=1) > 1e-12
    valid_labels = (
        torch.arange(num_classes, device=device)[valid_mask]
        if valid_mask.any()
        else None
    )
    ...
    for _ in range(epochs):
        for x, y in loader:
            ...
            if valid_labels is not None:
                p_out = model.classifier(consensus_P[valid_mask])
                loss_ce_proto = ce_loss(p_out, valid_labels)

            loss = loss_ce + mu * loss_ce_proto
    ...
    for _ in range(epochs):
        for x, y in loader:
            ...
            l_con = 0.0
            sample_mask = valid_mask[y]
            if sample_mask.any():
                target_protos = consensus_P[y[sample_mask]]
                l_con = mse_loss(features[sample_mask], target_protos)
