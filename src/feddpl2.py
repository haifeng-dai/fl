...
class Server(BaseServer):
    ...
    def fit(self):
        ...
        for r in range(self.rounds):
            ...
            for i in selected_clients:
                ...
                models_states.append(client_state)
            ...
            # 聚合各客户端学习到的 PLN 参数与特征提取器
            self.aggregate(models_states, plns_states, weights=norm_weights)
    ...
    def aggregate(self, model_params, pln_params, weights):
        # 1. 聚合 PLN 原型网络
        self.pln.load_state_dict(param_aggregate(pln_params, weights))

        # 2. 提取并聚合特征提取器 (extractor)
        extractor_params = [
            {k: v for k, v in state.items() if k.startswith("extractor")}
            for state in model_params
        ]
        global_extractor = param_aggregate(extractor_params, weights)

        # 3. 将聚合后的全局特征提取器同步给所有客户端，保留各客户端个性化的分类头 (classifier)
        for i in range(self.num_clients):
            for k, v in global_extractor.items():
                self.clients_state[i][k] = v.clone()
