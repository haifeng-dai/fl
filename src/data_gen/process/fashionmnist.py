...
def process(output_dir="./datasets/raw"):
    """
    下载并处理 FashionMNIST 数据集，合并训练集和测试集为 uint8 [0, 255]。
    """
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)

    train_set = datasets.FashionMNIST(root=output_dir, train=True, download=True)
    test_set = datasets.FashionMNIST(root=output_dir, train=False, download=True)
